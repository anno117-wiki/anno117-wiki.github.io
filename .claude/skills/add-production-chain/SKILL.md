...
description: Anno 117統合Wiki（`C:\Users\kojif\Desktop\anno_db2\`）に新しい生産チェーンJSONを追加、または既存チェーンを更新し、一覧再生成・型チェック・ビルド・E2Eまで検証を完了させる標準手順。数値の一次ソースは `_local/anno-official-data/official_master.csv`（GUID→公式日英名）と `assets.xml` であり、情報源不明の数値をタグなしで記載することは禁止されている。
---
