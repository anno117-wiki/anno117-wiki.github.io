...
# 局所効果2(副効果)の表示定義。公式データは「基準値 x 段階値」までしか分からず、
# 何がいくつ増減するかは実機確認済みのものだけをここに記す(2026-09-19確認)。
# 各行は (行の見出し, 段階値に掛ける倍率, 符号, 単位)。
#   エポナ: 必要な信仰1,500で 人口+2・名声+4 (人口+1x段階・名声+2x段階)
#   マルス: 必要な信仰300,000で 士気+70%・必要な労働力-70%・維持費-70% (段階値=%)
#   ネプトゥーヌス: 必要な信仰300,000で 維持費-70%・ヒットポイント+70% (段階値=%)
#   ウルカヌス: 必要な信仰250,000で 住民による労働力+35% (基準値5% x 段階値7)
# 未確認の効果は載せない = ページ側は素の「効果段階値」表示になる。
LOCAL2_DISPLAY = {
    'mars': [('士気', 1, '+', '%'), ('必要な労働力', 1, '-', '%'), ('維持費', 1, '-', '%')],
    'neptune': [('維持費', 1, '-', '%'), ('ヒットポイント', 1, '+', '%')],
    'ceres': [('追加の人口（住居ごと）', 1, '+', '')],
