...
- **wikiに反映済み（未コミット）**: `apps/wiki/docs/wiki/splendor.md`に「専門家のレース適性（隠しステータス）」セクションを新設し、プリセットのレンジ表（Common〜Legendary＋Talent版、計8行）を掲載。**`bun run build:site`でのビルド確認・`ls docs/`確認・コミットがまだ**（次セッションで実施すること）
...
- **`splendor.md`の未コミット分をビルド確認・コミット**（上記O。`bun run build:site`→`ls docs/`確認→コミット）
