# 引き継ぎ: 次回セッション向け（2026-09-20 更新・第9版）

## git状態
- ブランチ: master。コードの最新は `6975857`（build成果物）。その前の `2e2c50a`（比率表示）・`d713d98`（wiki注記）・`6247d15`（計算機注記）とこの引き継ぎ書（第9版）のコミットを含め、push 済みかは `git status -sb` で確認すること。デプロイは `gh run list` で確認
...
### L. 生産チェーン図に建物数の比率を表示（`2e2c50a`）
- `tools/build-chain-ratios.py`（要 `_local/anno-official-data/v2.1/assets.xml`）→ `apps/wiki/docs/wiki/chain-ratios.json`（62件）。表示は `ProductionChainSvg.vue`（ノード角の×Nバッジ＋図の下の注記）。データローダーは `production-chains.data.ts`
- 計算式: 根=1として、子の必要数 = 親の数 × (親の入力量/親サイクル) ÷ (子の産出量/子サイクル)。全建物が整数になる最小倍率（60以内）で掛ける。燃料は「燃料が要る建物×(炭焼き師サイクル30秒÷燃焼120秒)」
...
- 未確認: スマホ表示・ダークモード・E2E。経済ガイドの比率表は手書きのまま（比率JSONとは連動しない）
