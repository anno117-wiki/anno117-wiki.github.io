...
2. **タスク3（残課題・継続）**: アルビオン建物 `albion_underground_pit`「土窯」の公式名。
   - 忍者調査では「地窯」(Earth Oven, GUID 31767) の可能性が高いが英名表記の食い違いで殿の実機確認待ち。
   - `_local/anno-official-data/official_master.csv` で再突合も可。
