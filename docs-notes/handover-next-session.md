# 引き継ぎ: 次回セッション向け（2026-09-19 更新・第6版）

## git状態
- ブランチ: master。作業ツリーはクリーン。コードの最新は `562498f`（`origin/master` と一致・push済み）。**この引き継ぎ書の更新コミットのみ未push**（push可否はユーザーに確認）
- `562498f` のGitHub Pagesデプロイは成功済み（`gh run list --repo anno117-wiki/anno117-wiki.github.io --limit 5` で確認可）。本番サイトでの見た目確認は未実施
...
- **炭焼き師の燃焼時間120秒は計算機の定数を流用しており、公式データでの確認はしていない**（旧値 2:1:2:0.5 も同じ式で再現できたので整合はしている）
- 同ガイドの武器・ネックレス等の他の行は**v2.0の影響を未確認**
...
- 実画面確認済み: PC幅・800px幅・スマホ幅のハンバーガー。**未確認: ダークモード、ドロップダウンの実クリック操作、E2E（wikiのナビ文言への依存が無いことはgrepのみ）**
...
- 経済ガイド「100%効率の生産チェーン比率」の**パン以外の行**（武器・ネックレス等）がv2.0のサイクルタイム変更に対応しているか未確認（パンは訂正済み。F参照）。炭焼き師の数は燃焼時間120秒の前提
