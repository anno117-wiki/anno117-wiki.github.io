<template>
  ...
  <div
    class="tree-viewport"
    :style="`zoom: ${zoom};`"
    :class="{ 'is-panning': panState?.didMove }"
    ref="viewportEl"
    @mousedown="onViewportMousedown"
  >
    <ClientOnly>
      <TechLinks :branch="branch" :meta="meta" :techs="techs" :color="color" />
    </ClientOnly>
    <div class="tech-grid" :style="gridVars()">
      <div
        v-for="tech in techs"
        :key="tech.guid"
...
function gridStyle(tech: TechEntry) {
  const m = props.meta
  if (!m) return ''
...
function onWindowMousemove(e: MouseEvent) {
  if (!panState.value || !viewportEl.value) return
  ...
  viewportEl.value.scrollLeft = panState.value.scrollLeft - dx
  viewportEl.value.scrollTop = panState.value.scrollTop - dy
}
